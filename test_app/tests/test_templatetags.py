...
class ValueListTest(TestCase):
    def test_empty_list_should_show_none(self):
        out = Template("{% load mapentity_tags %}{% valuelist items %}").render(
            Context({"items": []})
        )
    ...
    def test_can_specify_field_to_be_used(self):
        ...
        out = Template(
            '{% load mapentity_tags %}{% valuelist items field="name" %}'
        ).render(Context({"items": [obj]}))
    ...
    def test_can_specify_an_enumeration4(self):
        out = Template(
            "{% load mapentity_tags %}{% valuelist items enumeration=True %}"
        ).render(Context({"items": range(1, 4)}))
    ...
    def test_can_specify_an_enumeration30(self):
        out = Template(
            "{% load mapentity_tags %}{% valuelist items enumeration=True %}"
        ).render(Context({"items": range(1, 30)}))
        self.assertIn('<li><span class="enumeration-value">AA.&nbsp;</span>1</li>', out)
        self.assertIn(
            '<li><span class="enumeration-value">AZ.&nbsp;</span>26</li>', out
        )
        self.assertIn(
            '<li><span class="enumeration-value">BA.&nbsp;</span>27</li>', out
        )
        self.assertIn(
            '<li><span class="enumeration-value">BB.&nbsp;</span>28</li>', out
        )

    def test_can_specify_an_enumeration300(self):
        out = Template(
            "{% load mapentity_tags %}{% valuelist items enumeration=True %}"
        ).render(Context({"items": range(1, 678)}))
        self.assertIn(
            '<li><span class="enumeration-value">AAA.&nbsp;</span>1</li>', out
        )
        self.assertIn(
            '<li><span class="enumeration-value">AAZ.&nbsp;</span>26</li>', out
        )
        self.assertIn(
            '<li><span class="enumeration-value">ABA.&nbsp;</span>27</li>', out
        )
        self.assertIn(
            '<li><span class="enumeration-value">ABB.&nbsp;</span>28</li>', out
        )
        self.assertIn(
            '<li><span class="enumeration-value">BAA.&nbsp;</span>677</li>', out
        )


class ValueTableTest(TestCase):
    def test_empty_objects_should_show_none(self):
        out = Template("{% load mapentity_tags %}{% valuetable dummys %}").render(
            Context({"dummys": DummyModel.objects.all()})
        )
    ...
    def test_simple_usage_outputs_list_of_items(self):
        ...
        out = Template(
            '{% load mapentity_tags %}{% valuetable dummys "name" %}'
        ).render(Context({"dummys": DummyModel.objects.all()}))
        self.assertHTMLEqual(
            out,
            f"""
...
class HumanizeTimesinceTest(TestCase):
    def test_no_date(self):
        out = Template("{% load mapentity_tags %}{{ date|timesince }}").render(
            Context({"date": ""})
        )
            ...
            Context(
                {
                    "date": make_aware(
                        datetime(2012, 6, 1)
                    )  # Initial import of Geotrek-admin on github
                }
            )
        )
    ...
    def test_year_ago(self):
        out = Template("{% load mapentity_tags %}{{ date|timesince }}").render(
            Context({"date": make_aware(datetime(2020, 11, 12))})
        )
        self.assertEqual(out, "1 year ago")

    def test_weeks_ago(self):
        out = Template("{% load mapentity_tags %}{{ date|timesince }}").render(
            Context({"date": make_aware(datetime(2021, 6, 12))})
        )
        self.assertEqual(out, "26 weeks ago")

    def test_week_ago(self):
        out = Template("{% load mapentity_tags %}{{ date|timesince }}").render(
            Context({"date": make_aware(datetime(2021, 12, 5))})
        )
        self.assertEqual(out, "1 week ago")

    def test_days_ago(self):
        out = Template("{% load mapentity_tags %}{{ date|timesince }}").render(
            Context({"date": make_aware(datetime(2021, 12, 10))})
        )
        self.assertEqual(out, "2 days ago")

    def test_day_ago(self):
        out = Template("{% load mapentity_tags %}{{ date|timesince }}").render(
            Context({"date": make_aware(datetime(2021, 12, 11))})
        )
        self.assertEqual(out, "1 day ago")

    def test_hours_ago(self):
        out = Template("{% load mapentity_tags %}{{ date|timesince }}").render(
            Context({"date": make_aware(datetime(2021, 12, 12, 9))})
        )
        self.assertEqual(out, "15 hours ago")

    def test_hour_ago(self):
        out = Template("{% load mapentity_tags %}{{ date|timesince }}").render(
            Context({"date": make_aware(datetime(2021, 12, 12, 23))})
        )
        self.assertEqual(out, "1 hour ago")

    def test_minutes_ago(self):
        out = Template("{% load mapentity_tags %}{{ date|timesince }}").render(
            Context({"date": make_aware(datetime(2021, 12, 12, 23, 55))})
        )
        self.assertEqual(out, "5 minutes ago")

    def test_minute_ago(self):
        out = Template("{% load mapentity_tags %}{{ date|timesince }}").render(
            Context({"date": make_aware(datetime(2021, 12, 12, 23, 59))})
        )
        self.assertEqual(out, "1 minute ago")

    def test_few_seconds_ago(self):
        out = Template("{% load mapentity_tags %}{{ date|timesince }}").render(
            Context({"date": make_aware(datetime(2021, 12, 12, 23, 59, 59))})
        )
...
class SmartIncludeTest(TestCase):
    def test_smart_include_no_argument(self):
        with self.assertRaisesRegex(
            TemplateSyntaxError, "'smart_include' tag requires one argument"
        ):
            Template("{% load mapentity_tags %}{% smart_include %}").render(Context())
    ...
    def test_smart_include_no_quotes(self):
        with self.assertRaisesRegex(
            ...
            Template("{% load mapentity_tags %}{% smart_include test %}").render(
                Context()
            )
...
class LatLngBoundsTest(TestCase):
    def test_latlngbound_null(self):
        out = Template("{% load mapentity_tags %}{{ object|latlngbounds }}").render(
            Context({"object": None})
        )
    ...
    def test_latlngbound_geosgeometry(self):
        ...
        out = Template("{% load mapentity_tags %}{{ geom|latlngbounds }}").render(
            Context({"geom": geom})
        )
        json_out = json.loads(out)
...
class FieldVerboseNameTest(TestCase):
    def test_field_no_field_but_verbose_name_field(self):
        ...
        template = Template(
            '{% load mapentity_tags %}{{ object|verbose:"do_not_exist" }}'
    ...
    def test_field_verbose_name_field_does_not_exist(self):
        object_dummy = DummyModelFactory.create()
        with self.assertRaisesRegex(
            FieldDoesNotExist, "DummyModel has no field named 'do_not_exist'"
        ):
            Template(
                '{% load mapentity_tags %}{{ object|verbose:"do_not_exist" }}'
            ).render(Context({"object": object_dummy}))
