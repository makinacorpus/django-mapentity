...
class ZipShapeSerializer(Serializer):
    ...
    def _create_shape(self, shape_directory, queryset, model, columns):
        ...
        if geom_type.upper() in (
            GeometryField.geom_type,
            GeometryCollectionField.geom_type,
        ):
            (
                by_points,
                by_linestrings,
            ...
            for split_qs, split_geom_field in (
                (by_points, PointField),
            ...
            shape_write(
                shape_directory, queryset, model, columns, get_geom, geom_type, srid
            )
    ...
    def split_bygeom(self, iterable, geom_getter=lambda x: x.geom):
        ...
        for x in iterable:
            ...
            if geom is None:
                ...
                msg = f"Only LineString, Point and Polygon should be here. Got {geom} for pk {x.pk}"
...
def shape_write(
    shape_directory, iterable, model, columns, get_geom, geom_type, srid, srid_out=None
):
    """
    ...
    shape, headers = create_shape_format_layer(
        shape_directory, headers, geom_type, srid, srid_out
    )
            ...
            shape.write(
                {
                    ...
                    "properties": get_serialized_properties(
                        model, item, columns, columns_headers
                    ),
                }
            )
    shape.close()
...
def get_serialized_properties(model, item, columns, columns_headers):
    ...
    for fieldname in columns:
        ...
        if isinstance(modelfield, ForeignKey):
            properties[columns_headers[fieldname][:10]] = smart_plain_text(
                getattr(item, fieldname)
            )
        elif isinstance(modelfield, ManyToManyField):
            properties[columns_headers[fieldname][:10]] = ",".join(
                [smart_plain_text(o) for o in getattr(item, fieldname).all()] or ""
            )
        else:
            properties[columns_headers[fieldname][:10]] = field_as_string(
                item, fieldname
            )
    return properties
...
def create_shape_format_layer(directory, headers, geom_type, srid, srid_out=None):
    ...
    shape = fiona.open(
        os.path.join(directory, f"{geom_type}.shp"),
        layer=geom_type,
        mode="w",
...
def geo_field_from_model(model, default_geo_field_name=None):
    ...
    if len(geo_fields) > 1:
        if not default_geo_field_name:
            msg = (
                f"More than one geodjango geometry field found, please specify which to use by name using \
the 'geo_field' keyword. Available fields are: '{geo_fields_names()}'"
            )
            raise ValueError(msg)
        else:
            geo_field_by_name = [
                fld for fld in geo_fields if fld.name == default_geo_field_name
            ]
            if not geo_field_by_name:
                msg = f"Geodjango geometry field not found with the name '{default_geo_field_name}', fields available are: '{geo_fields_names()}'"
                raise ValueError(msg)
            else:
        ...
        msg = "No geodjango geometry fields found in this model"
        raise ValueError(msg)
