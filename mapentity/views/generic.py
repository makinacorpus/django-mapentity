...
class MapEntityFormat(BaseListView, ListView):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        if not formatter:
            msg = f"Unknown serialization format '{fmt_str}'"
            logger.warning(msg)
        ...
        filename += f".{extensions.get(fmt_str, fmt_str)}"
...
class MapEntityDocumentBase(ModelViewMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["objecticon"] = os.path.join(
            settings.STATIC_ROOT, self.get_entity().icon_big
        )
...
class MapEntityWeasyprint(MapEntityDocumentBase):
    def __init__(self, *args, **kwargs):
        ...
        self.template_attributes = smart_get_template(
            self.model, suffix_for(self.template_name_suffix, "_attributes", "html")
        )
        self.template_css = smart_get_template(
            self.model, suffix_for(self.template_name_suffix, "_pdf", "css")
        )
...
class MapEntityDocumentOdt(MapEntityDocumentBase):
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.with_html_attributes:
            context["attributeshtml"] = self.get_object().get_attributes_html(
                self.request
            )
        context["_"] = _
        return context
...
class Convert(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        received = download_content(url, silent=True, headers=self.request_headers())
        if received:
...
class MapEntityDuplicate(ModelViewMixin, SingleObjectMixin, View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if not clone:
                messages.error(
                    self.request, _("Duplication is not available for this object")
                )
                return HttpResponseRedirect(original_object.get_detail_url())
            log_action(self.request, clone, ADDITION)
            messages.success(
                self.request,
                f"{self.get_object()._meta.verbose_name} "
                + gettext("has been duplicated successfully"),
            )
            return HttpResponseRedirect(clone.get_detail_url())
        except Exception as exc:
            msg = f"An error occurred during duplication {exc}"
            logger.warning(msg)
...
class MapEntityDetail(ModelViewMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        logentries = LogEntry.objects.filter(
            content_type_id=self.object.get_content_type_id(), object_id=self.object.pk
        ).order_by("-id")
        ...
        perm_update = self.get_model().get_permission_codename(
            mapentity_models.ENTITY_UPDATE
        )
        can_edit = user_has_perm(self.request.user, perm_update)
        context["can_edit"] = can_edit
        perm_create = self.get_model().get_permission_codename(
            mapentity_models.ENTITY_CREATE
        )
        can_add = user_has_perm(self.request.user, perm_create)
        context["can_add"] = can_add
...
class MapEntityUpdate(ModelViewMixin, FormViewMixin, UpdateView):
    ...
    def get_form_kwargs(self):
        ...
        perm_delete = self.get_model().get_permission_codename(
            mapentity_models.ENTITY_DELETE
        )
