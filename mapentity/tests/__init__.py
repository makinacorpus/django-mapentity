...
class MapEntityTest(TestCase):
    ...
    def test_document_markup(self, mock_requests):
        ...
        response = self.client.get(
            reverse(obj._entity.url_name(ENTITY_MARKUP), args=[obj.pk])
        )
        self.assertEqual(response.status_code, 200)
    ...
    def _post_form(self, url):
        ...
        for err in form_errors:
            self.assertTrue(
                f"{err}" in fields_errors, f"'{err}' not in {fields_errors}"
            )
    ...
    def test_duplicate(self):
        ...
        user.user_permissions.add(
            Permission.objects.get(codename=f"add_{obj._meta.model_name}")
        )
        self.client.force_login(user=user)
        ...
        self.assertIn(
            f"{self.model._meta.verbose_name} has been duplicated successfully", msg
        )
    ...
    def test_api_datatables_list_for_model(self):
        ...
        self.assertEqual(
            content_json,
            {
                "data": [self.get_expected_datatables_attrs()],
                "draw": 1,
                "recordsFiltered": 1,
                "recordsTotal": 1,
            },
    ...
    def test_api_no_format_list_for_model(self):
        ...
        self.obj = self.modelfactory.create()
        list_url = (
            f"/api/{self.model._meta.model_name}/drf/{self.model._meta.model_name}s"
        )
        response = self.client.get(list_url)
        self.assertEqual(response.status_code, 200, f"{list_url} not found")
        content_json = response.json()
    ...
    def test_api_geojson_detail_for_model(self):
        ...
        self.obj = self.modelfactory.create()
        detail_url = f"/api/{self.model._meta.model_name}/drf/{self.model._meta.model_name}s/{self.obj.pk}.geojson"
    ...
    def test_api_geojson_list_for_model(self):
        ...
        self.obj = self.modelfactory.create()
        list_url = f"/api/{self.model._meta.model_name}/drf/{self.model._meta.model_name}s.geojson"
        response = self.client.get(list_url)
        self.assertEqual(response.status_code, 200, f"{list_url} not found")
        content_json = response.json()
        self.assertEqual(
...
class MapEntityLiveTest(LiveServerTestCase):
    ...
    def login(self):
        ...
        response = self.client.get(login_url, allow_redirects=False)
    ...
    def test_geojson_cache(self, latest_updated):
        ...
        self.assertCountEqual(
            cachecontrol.split(", "), ("must-revalidate", "max-age=0")
        )
        ...
        response = self.client.get(
            geojson_layer_url, HTTP_IF_MODIFIED_SINCE=lastmodified
        )
        self.assertEqual(response.status_code, 304)

        # Ask again with headers in the past, and expect a 200
        response = self.client.get(
            geojson_layer_url, HTTP_IF_MODIFIED_SINCE=http_date(1000)
        )
        self.assertEqual(response.status_code, 200)
        new_hasher = hashlib.md5()
        new_hasher.update(response.content)
    ...
    def test_map_image(self, mock_requests):
        ...
        screenshot_url = f"http://0.0.0.0:8001/?url={quote(mapimage_url)}"
