    ...
    def replace_orig_fields(self):
        ...
        for modelfield in mto.fields:
            ...
            for lang in settings.MODELTRANSLATION_LANGUAGES:
                ...
                translated.label = f"{translated.label} [{lang}]"
...
class SubmitButton(HTML):
    def __init__(self, div_id, label):
        ...
        content = f"""
            <button type="submit" id="{div_id}" class="btn btn-success"
...
class MapEntityForm(TranslatedModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if max_characters:
            ...
            global_help_text = _("%(max)s characters maximum recommended") % {
                "max": max_characters
            }

        max_characters_by_field_config = (
            settings.MAPENTITY_CONFIG.get("MAX_CHARACTERS_BY_FIELD", {}) or {}
        )
        # Default widgets
                    ...
                    if needs_replace_widget:
                        ...
                        if self.instance.pk and self.user:
                            if not self.user.has_perm(
                                self.instance.get_permission_codename(
        ...
        if self.instance.pk and self.user:
            if not self.user.has_perm(
                self.instance.get_permission_codename(ENTITY_PERMISSION_UPDATE_GEOM)
            ):
    ...
    def _init_layout(self):
        ...
        actions = [
            ...
            SubmitButton(
                "save_changes", _("Create") if is_creation else _("Save changes")
            ),
        ]
        ...
        if not is_creation:
            ...
            actions.insert(
                0,
                HTML(
                    """<a class="btn {} delete" href="{}"><i class="bi bi-trash"></i> {}</a>""".format(
        ...
        self.helper.layout = Layout(
            Div(
                Div(leftpanel, *rightpanel, css_class="row"),
    ...
    def __tabbed_layout_for_field(self, field):
        ...
        for replacement in self._translated[field]:
            active = (
                ...
                if replacement.endswith(
                    "_{}".format(
                        settings.MODELTRANSLATION_DEFAULT_LANGUAGE.replace("-", "_")
                    )
                )
                else ""
            )
            fields.append(
                Div(replacement, css_class="tab-pane " + active, css_id=replacement)
            )
        ...
        layout = Div(
            ...
            """.format(
                    lang_code=settings.MODELTRANSLATION_DEFAULT_LANGUAGE.replace(
                        "-", "_"
                    ),
                    field=field,
                )
            ),
            Div(*fields, css_class="tab-content"),
