    ...
    def duplicate(self, **kwargs):
        ...
        try:
            ...
            if not skip_attachments:
                for attachment in get_attachment_model().objects.filter(
                    object_id=self.pk, content_type_id=self.get_content_type_id()
                ):
...
class BaseMapEntityMixin(DuplicateMixin, models.Model):
    ...
    def get_permission_codename(cls, entity_kind):
        ...
        if opts.proxy:
            ...
            appname = (
                proxied.app_label.lower()
                if proxied.app_label.lower() != "admin"
                else appname
            )
    ...
    def get_popup_url(self):
        modelname = cls._meta.model_name.lower()
    ...
    def prepare_map_image(self, rooturl):
        ...
        if self.get_geom() is None:
            ...
            draw.text(
                (10, 10), "This object has no geometry", font=font, fill=(0, 0, 0)
            )
            image.save(path)
            return True
        ...
        if length:
            ...
            size = app_settings["MAP_CAPTURE_SIZE"]
        printcontext = (
            self.get_printcontext() if hasattr(self, "get_printcontext") else None
        )
        capture_map_image(
            url,
            path,
    ...
    def get_map_image_path(self):
        ...
        return os.path.join("maps", f"{self._meta.model_name}-{self.pk}.png")
...
class LogEntry(BaseMapEntityMixin, BaseLogEntry):
    ...
    class Meta:
        proxy = True
        app_label = "mapentity"
        permissions = (("read_logentry", "Can read log entries"),)
    ...
    def action_time_display(self):
        return f"{localize(self.action_time)} ({humanize_timesince(self.action_time)})"
    ...
    def object_display(self):
        ...
        try:
            ...
            assert obj._entity, f"Unregistered model {model_str}"
            obj_url = obj.get_detail_url()
        except (ObjectDoesNotExist, NoReverseMatch, AssertionError):
            return f"{model_str} {self.object_repr}"
        else:
            return f'<a data-pk="{obj.pk}" href="{obj_url}" >{model_str} {self.object_repr}</a>'
